...
interface InlineAssigneePopoverProps {
  // tareaId is kept for future use (e.g., permissions)
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  tareaId: string | null
  organizacionId: string
  assignedMiembro: MiembroOrganizacion | null
  onAssign: (miembroId: string) => Promise<void>
...
/**
 * InlineAssigneePopover - Popover-based assignee selector with rich display
 *
 * Features:
 * - Closed state: Shows Avatar + Name + Email (rich display)
 * - Open state: Searchable member list with debounced input
 * - Auto-save on selection, immediate close
 * - Option to unassign (clear assignment)
 */
            ...
            <div className="flex items-center gap-3 w-full">
              <Avatar className="h-8 w-8 border border-slate-200/60 shadow-sm shrink-0">
                <AvatarFallback className="text-[10px] bg-primary/10 text-primary font-semibold">
                  {getInitials(assignedMiembro.nombres, assignedMiembro.email)}
                </AvatarFallback>
              </Avatar>
              <div className="flex flex-col items-start min-w-0 flex-1">
                <span className="text-sm font-medium truncate">
                  {assignedMiembro.nombres} {assignedMiembro.apellidos}
                </span>
                <span className="text-xs text-muted-foreground truncate">
                  {assignedMiembro.email}
                </span>
              </div>
            </div>
          ) : (
            ...
            searchResults.map((miembro) => (
              <button
                ...
                className={cn(
                  ...
                  assignedMiembro?.user_id === miembro.user_id && "bg-accent",
              ...
              >
                ...
                {assignedMiembro?.user_id === miembro.user_id && (
