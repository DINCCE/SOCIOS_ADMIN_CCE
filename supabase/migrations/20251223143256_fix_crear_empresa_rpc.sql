-- ========================================
-- ADD email_secundario TO empresas
-- ========================================

ALTER TABLE empresas 
ADD COLUMN IF NOT EXISTS email_secundario TEXT;

-- Constraint for email_secundario format in empresas
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_empresa_email_secundario_format') THEN
    ALTER TABLE empresas
    ADD CONSTRAINT check_empresa_email_secundario_format 
    CHECK (email_secundario IS NULL OR email_secundario ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$');
  END IF;
END $$;

-- ========================================
-- FIXED API RPC: crear_empresa
-- ========================================
...
BEGIN
  ...
  -- 3. Validar unicidad (excluyendo eliminados)
  IF EXISTS (
