...
import { Expand, Table as TableIcon, LineChart } from "lucide-react"
...
import { BalanceChartExpanded } from "./balance-chart-expanded"

// Feature flag for easy rollback
const ENABLE_EXPANDABLE_CHART = true
...
export function BalanceChart({ tareas }: BalanceChartProps) {
    ...
    // Keyboard shortcut for expanding chart (CMD+Shift+E or Ctrl+Shift+E)
    useEffect(() => {
        if (!ENABLE_EXPANDABLE_CHART) return
    ...
    return (
        <>
            <Card className="border border-border shadow-sm bg-card group relative">
                {ENABLE_EXPANDABLE_CHART && (
        ...
        {ENABLE_EXPANDABLE_CHART && (
            <ChartModal
                ...
                title="Balance de Tareas"
                subtitle="Vista detallada de los últimos 30 días"
                actions={
                    <Button
                        variant="ghost"
                        size="icon"
                        ...
                        aria-label={activeTab === 'chart' ? 'Ver tabla' : 'Ver gráfico'}
                    >
                        {activeTab === 'chart' ? (
            ...
            >
                <BalanceChartExpanded tareas={tareas} activeTab={activeTab} />
            </ChartModal>
        )}
    </>
    )
