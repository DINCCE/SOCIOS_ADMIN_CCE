...
import { columns, type DocumentoComercialView } from '@/features/procesos/documentos-comerciales/columns'
import {
  docComercialesEstadoOptions,
  docComercialesTipoOptions,
  docComercialesSubTipoOptions,
  getDocComercialTagsOptions
} from '@/lib/table-filters'
import { calculateDefaultPageSize } from '@/lib/utils/pagination'
import { useDataExport } from '@/lib/hooks/use-data-export'
import { useNotify } from '@/lib/hooks/use-notify'
import { toggleTagsForDocumentos, createAndAssignTagForDocumentos } from '@/app/actions/tags'
import { softDeleteDocComercial } from '@/app/actions/doc-comerciales'

export function DocComercialesPageClient() {
    ...
    return data.filter((item) => {
      return columnFilters.every((filter) => {
        ...
        if (Array.isArray(value)) {
          // Multi-select filter (e.g., estado, tipo, sub_tipo_filter)
          if (id === 'tags') {
  ...
  const handleExport = ({ format, selectedColumns }: { format: 'csv' | 'xlsx'; selectedColumns: string[] }) => {
    exportData(filteredData, {
      ...
      selectedColumns,
      filename: `documentos-comerciales-${Date.now()}`
    })
  }
  ...
  const handleExportSelection = ({ format, selectedColumns }: { format: 'csv' | 'xlsx'; selectedColumns: string[] }) => {
    ...
    exportData(selectedRows, {
      ...
      columns: exportColumns,
      selectedColumns,
      filename: `seleccion_documentos_${Date.now()}`
    })
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const result = await softDeleteDocComercial(id)
        if (result.success) {
          successCount++
        } else {
          errorCount++
      ...
      setRowSelection({})
      await queryClient.invalidateQueries({ queryKey: ['doc_comercial'] })
      setShowDeleteConfirm(false)

      // Show appropriate notification
      if (errorCount === 0) {
        notifySuccess({
          title: `${successCount} ${successCount === 1 ? 'documento eliminado' : 'documentos eliminados'} correctamente`
        })
      } else if (successCount === 0) {
        notifyError({
          title: 'Error al eliminar',
          description: `No se pudieron eliminar los documentos. ${errorCount > 1 ? 'Intente nuevamente.' : ''}`
        })
      } else {
        notifyError({
          title: 'Eliminación parcial',
          description: `${successCount} de ${selectedCount} documentos eliminados. ${errorCount} errores.`
        })
      }
    } catch (error) {
      ...
      notifyError({
        title: 'Error al eliminar',
        description: 'Error al eliminar los documentos. Intente nuevamente.'
      })
      setShowDeleteConfirm(false)
    }
  }
    ...
    <PageShell>
        ...
        left={
          <>
            ...
            <DataTableFacetedFilter
              ...
              title="Estado"
              options={docComercialesEstadoOptions}
            />
            <DataTableFacetedFilter
              column={table.getColumn("tipo_filter")}
              title="Tipo"
              options={docComercialesTipoOptions}
            />
            <DataTableFacetedFilter
              column={table.getColumn("sub_tipo_filter")}
              title="Subtipo"
              options={docComercialesSubTipoOptions}
            />
            <DataTableFacetedFilter
              column={table.getColumn("tags")}
              title="Etiquetas"
              options={getDocComercialTagsOptions(initialData)}
            />
      ...
      <PageContent>
        {view === 'board' ? (
          ...
          <DocComercialesBoard data={fullyFilteredData} initialData={initialData} />
        ) : (
          /* TABLA: Comportamiento natural con scroll */
          <div className="space-y-4">
            {/* Table - with responsive card view on mobile */}
            <ResponsiveDocComercialesDataTable table={table} router={router} />
            {/* Pagination Footer */}
        ...
        <AnimatePresence>
          {table.getFilteredSelectedRowModel().rows.length > 0 && (
            <FloatingActionCapsule
              ...
              selectedRowsTags={table.getFilteredSelectedRowModel().rows.map((row: any) => (row.original as DocumentoComercialView).tags || [])}
              onClearSelection={() => setRowSelection({})}
              onExport={() => setShowSelectionExport(true)}
              onToggleTag={async (tag: string, add: boolean) => {
                const selectedIds = table.getFilteredSelectedRowModel().rows.map((row: any) => row.original.id)
                const result = await toggleTagsForDocumentos(selectedIds, tag, add)
                if (!result.success) {
                  console.error('Error toggling tag:', result.message)
                }
                await queryClient.invalidateQueries({ queryKey: ['doc_comerciales'] })
              }}
              onCreateTag={async (tag: string) => {
                const selectedIds = table.getFilteredSelectedRowModel().rows.map((row: any) => row.original.id)
                const result = await createAndAssignTagForDocumentos(selectedIds, tag)
                if (!result.success) {
